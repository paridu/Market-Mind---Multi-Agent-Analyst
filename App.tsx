...
const App: React.FC = () => {
  ...
  const handleSelectTech = (item: TechStockItem) => {
    const text = `TECH SCANNER: ${item.name} (${item.ticker})
ราคาล่าสุด: ${item.current_price}
แนวรับ (Support): ${item.support_level} | แนวต้าน (Resistance): ${item.resistance_level}
เป้าหมาย 1Q: ${item.target_1q} (Upside ${item.upside})
...
คะแนนรวม (Total Score): ${item.ahp_score}/100

ปัจจัย 4 ด้าน (4 Factors):
1. การเติบโต (Growth): ${item.factors.growth}/10
2. ความคุ้มค่า (Value): ${item.factors.value}/10
3. โมเมนตัม (Momentum): ${item.factors.momentum}/10
4. คุณภาพ (Quality): ${item.factors.quality}/10

ลำดับที่ #${item.rank} (S&P 500 Selection)
เหตุผล: ${item.reasoning}`;
    setNewsText(text);
  };
...
Action: ${item.action}
คุณภาพพื้นฐาน (Fundamental): ${item.importance}/100
ความน่าสนใจทางเทคนิค (Technical): ${item.urgency}/100
เหตุผล: ${item.reason}`;
    setNewsText(text);
  };
