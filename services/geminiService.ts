    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          price_trend: { 
             type: Type.STRING, 
             description: "Brief description of the stock's recent price movement (e.g. 'Up 10% last month', 'Near 52-week high'). Infer from news or use general knowledge." 
          },
        ...
        required: ["ticker", "category", "price_trend", "thesis", "what_to_check", "action"]
      }
    }
  });
...
export async function runMatrixAnalysis(): Promise<MatrixItem[]> {
  const prompt = `
    Perform a multi-dimensional analysis on 20 trending US Tech/Growth stocks to place them on an Eisenhower Matrix (2x2 Grid).
    ...
    Return a JSON array of 20 items.
    IMPORTANT: Return raw JSON. No Markdown.
  `;

  // Switched to Pro for better reasoning on 20 items and removed strict schema for stability
  const response = await ai.models.generateContent({
    model: 'gemini-3-pro-preview',
    contents: prompt,
    config: {
  ...
  if (!response.text) throw new Error("Matrix analysis failed (Empty response).");
  ...
  try {
      ...
      return JSON.parse(cleanJson) as MatrixItem[];
  } catch (e) {
      console.error("JSON Parse Error in Matrix Analysis:", e, response.text);
      throw new Error("Matrix analysis failed (Invalid JSON).");
  }
}